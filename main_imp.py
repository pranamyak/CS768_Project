...
def train_get_mask(MODEL_NAME, dataset_ori, net_params, things_dict, imp_num, filename, pruned_filename, args):
    ...
    trainset_ori, valset_ori, biased_testset_ori, unbiased_testset_ori, unseen_testset_ori = dataset_ori.train, dataset_ori.val, dataset_ori.biased_test, dataset_ori.unbiased_test, dataset_ori.unseen_test
    ...
    unseen_test_loader_ori = DataLoader(unseen_testset_ori, batch_size=net_params["batch_size"], shuffle=False, drop_last=False, collate_fn=dataset_ori.collate)
    ...
    if is_things_dict:
        ...
        unseen_testset_pru = copy.deepcopy(unseen_testset_ori)

        train_loader_pru = DataLoader(trainset_pru, batch_size=net_params["batch_size"], shuffle=True, drop_last=False, collate_fn=dataset_ori.train_collate)
        val_loader_pru = DataLoader(valset_pru, batch_size=net_params["batch_size"], shuffle=False, drop_last=False, collate_fn=dataset_ori.collate)
        biased_test_loader_pru = DataLoader(biased_testset_pru, batch_size=net_params["batch_size"], shuffle=False, drop_last=False, collate_fn=dataset_ori.collate)
        unbiased_test_loader_pru = DataLoader(unbiased_testset_pru, batch_size=net_params["batch_size"], shuffle=False, drop_last=False, collate_fn=dataset_ori.collate)
        unseen_test_loader_pru = DataLoader(unseen_testset_pru, batch_size=net_params["batch_size"], shuffle=False, drop_last=False, collate_fn=dataset_ori.collate)
    ...
    sp_unseen_test = pruning.print_pruning_percent(unseen_test_loader_ori, unseen_test_loader_pru, "unbiased")
    ...
    best_unbiased_test_acc, best_biased_test_acc, best_unseen_test_acc = 0, 0, 0
    print("done! cost time:[{:.2f} min]".format((time.time() - t0) / 60))

    print("inside train get mask")
    for epoch in range(args.mask_epochs):
        ...
        _, epoch_unseen_test_acc = eval_acc_with_mask(model_c, masker_c, model_b, device, unseen_test_loader_pru, epoch, args)    
        ...
        if epoch_val_acc > best_val_acc:
            ...
            update_unseen_test_acc = epoch_unseen_test_acc
            best_epoch = epoch
        ...
        if epoch_unseen_test_acc > best_unseen_test_acc:
            best_unseen_test_acc = epoch_unseen_test_acc

        
        print('-'*120)
        str1 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + ' | ' + 'Train IMP:[{}] spa[{:.2f}%] spw:[{:.2f}%] | Epoch [{}/{}] Loss:[{:.4f}] Train:[{:.2f}] Val:[{:.2f}] Biased Test:[{:.2f}] Update Biased Test:[{:.2f}] Best Biased Test:[{:.2f}]| Unbiased Test:[{:.2f}]  Update Unbiased Test:[{:.2f}] Best Unbiased Test:[{:.2f}]| Unseen Test:[{:.2f}]  Update Unseen Test:[{:.2f}] Best Unseen Test:[{:.2f}]| epoch:[{}] | Time:[{:.2f} min] | [{:.2f}%] [{:.2f}%] [{:.2f}%] [{:.2f}%] [{:.2f}%]'.format(imp_num,
                        spa * 100,
                        spw * 100,
                        epoch + 1, 
                        args.mask_epochs,
                        ...
                        epoch_unseen_test_acc * 100, 
                        update_unseen_test_acc * 100,
                        best_unseen_test_acc * 100,
                        best_epoch,
                        run_time / 60,
        ...
        print( time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) + ' | ' +
                'Train IMP:[{}] spa[{:.2f}%] spw:[{:.2f}%] | Epoch [{}/{}] Loss:[{:.4f}] Train:[{:.2f}] Val:[{:.2f}] Biased Test:[{:.2f}] Update Biased Test:[{:.2f}] Best Biased Test:[{:.2f}]| Unbiased Test:[{:.2f}]  Update Unbiased Test:[{:.2f}] Best Unbiased Test:[{:.2f}] | Unseen Test:[{:.2f}]  Update Unseen Test:[{:.2f}] Best Unseen Test:[{:.2f}] epoch:[{}] | Time:[{:.2f} min] | [{:.2f}%] [{:.2f}%] [{:.2f}%] [{:.2f}%] [{:.2f}%]'
                .format(imp_num,
                        spa * 100,
                        spw * 100,
                        epoch + 1, 
    ...
    print("INFO: Data Sparsity:[{:.2f}%]".format(spa * 100))
    ...
    things_dict['unseen_testset_pru'] = unseen_testset_pru 
