...
class SuperPixDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name): #name = MNIST_75sp_0.8
        """
    ...
    def train_collate(self, samples):
        ...
        graphs, labels = map(list, zip(*samples))
        labels = torch.tensor(np.array(labels))
        #bias_labels = torch.tensor(np.array(bias_labels))
        #vector_c = torch.tensor(np.array(vector_c))

        #vector_b = torch.tensor(np.array(vector_b))
        
        #index = torch.tensor(np.array(index))
        for idx, graph in enumerate(graphs):
            graphs[idx].ndata['feat'] = graph.ndata['feat'].float()
            #graphs[idx].edata['feat'] = graph.edata['feat'].float()
        batched_graph = dgl.batch(graphs)
