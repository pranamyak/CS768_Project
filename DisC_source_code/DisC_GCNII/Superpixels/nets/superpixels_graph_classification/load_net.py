...
from nets.superpixels_graph_classification.gcn_net import GCNNet, GCNMasker, IRMNet, GroupDRONet, HRMNet 
...
def IRM(net_params):
    return IRMNet(net_params)
def GroupDRO(net_params):
    return GroupDRONet(net_params)

def HRM(net_params):
    return HRMNet(net_params)
