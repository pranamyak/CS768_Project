...
def train_get_mask(dataset_ori, net_params, things_dict, imp_num, filename, pruned_filename, args):
    t0 = time.time()
    print("process ...")
    ...
    model_c = gnn_model("GCNII", net_params).to(device)
    masker_c = mask_model(net_params).to(device)
    
    model_b = gnn_model("GCNII", net_params).to(device)
...
def main():  
    ...
    #    args.pa = 0.05
    #    args.pw = 0.2
        things_dict = train_get_mask(dataset, net_params, things_dict, imp_num, filename, pruned_filename, args)
