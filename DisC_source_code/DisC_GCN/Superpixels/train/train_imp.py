...
def train_model_and_masker(model_c, masker_c,  model_b, masker_b, optimizer_c, optimizer_b, sample_loss_ema_c, sample_loss_ema_b, device, data_loader, epoch, args):
    ...
    for iter, (batch_graphs, batch_labels) in enumerate(data_loader):
        ...
        if not args.use_mask:
            data_mask_c = None
            data_mask_b = None
        batch_scores_c = model_c(batch_graphs, batch_x, batch_labels, None, data_mask_c, data_mask_node_c, device)
        batch_scores_b = model_b(batch_graphs, batch_x, batch_labels, None, data_mask_b, data_mask_node_b, device)
    ...
    with torch.no_grad():
        for iter, (batch_graphs, batch_labels) in enumerate(data_loader):
            ...
            if not args.use_mask:
                data_mask_c = None
                data_mask_b = None
            batch_scores_c = model_c.test(batch_graphs, batch_x, batch_e, data_mask_c, data_mask_node_c)

            batch_scores_b = model_b.test(batch_graphs, batch_x, batch_e, data_mask_b, data_mask_node_b)
