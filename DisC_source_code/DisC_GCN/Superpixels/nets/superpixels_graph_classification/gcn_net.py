...
class GCNNet(nn.Module):
    def __init__(self, net_params):
        super().__init__()
        self.gcn_net = GCNNet_1(net_params)
    ...
    def forward(self, batch_graphs, batch_x, batch_labels, envs, data_mask, data_mask_node, device):
        ...
        results = self.gcn_net(batch_graphs, batch_x, None, data_mask, None)
        #print("penalty", avg_loss,penalty)
        return results

    def test(self, g, h, e, data_mask=None, data_mask_node=None):
        results = self.gcn_net(g, h, None, data_mask, None)
        return results

class GCNNet_1(nn.Module):
    def __init__(self, net_params):
        super().__init__()
