...
def train_get_mask(dataset_ori, net_params, things_dict, imp_num, filename, args):
    ...
    model_c = gnn_model("GIN", net_params).to(device)
    masker_c = mask_model(net_params).to(device)

    #net_params['L'] = 4
    #net_params['n_mlp_GIN'] = 2
    model_b = gnn_model("GIN", net_params).to(device)
    ...
    optimizer_c = optim.Adam([{'params': model_c.parameters(), 'lr': 0.001}, 
                            {'params': masker_c.parameters(),'lr': 0.001}], weight_decay=0)

    scheduler_c = optim.lr_scheduler.ReduceLROnPlateau(optimizer_c, mode='min',factor=0.5, patience=20, verbose=True)               
    optimizer_b = optim.Adam([{'params': model_b.parameters(), 'lr': 0.001}, 
                            {'params': masker_b.parameters(),'lr': 0.001}], weight_decay=0)
    ...
    model_mask_dict_c = pruning.extract_mask(model_c)

    model_mask_dict_b = pruning.extract_mask(model_b)
    # masker.load_state_dict(best_masker_state_dict)
    t0 = time.time()
    trainset_pru = pruning.masker_pruning_dataset_train(train_loader_pru, masker_c, device, args)
    valset_pru = pruning.masker_pruning_dataset(val_loader_pru, masker_c, device, args)
    unbiased_testset_pru = pruning.masker_pruning_dataset(unbiased_test_loader_pru, masker_c, device, args)
    biased_testset_pru = pruning.masker_pruning_dataset(biased_test_loader_pru, masker_c, device, args)
    t1 = time.time()
    sp_train = pruning.print_pruning_percent(train_loader_ori, train_loader_pru, "train")
    sp_val = pruning.print_pruning_percent(val_loader_ori, val_loader_pru, "val")
    sp_biased_test = pruning.print_pruning_percent(biased_test_loader_ori, biased_test_loader_pru, "biased")
...
def main():  
    ...
    dataset = LoadData(DATASET_NAME, args.dataset)
    ...
    net_params['learn_eps_GIN'] = args.learn_eps_GIN
