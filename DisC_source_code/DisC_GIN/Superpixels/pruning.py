...
def parser_loader():
    ...
    parser.add_argument('--learn_eps_GIN', type=float, default=1.)
...
def pruning_batch_data_from_mask(data_list, batch_labels, bias_labels, data_mask, node_masker, args):

    offset = 0
    nodes_offset = 0
    new_data_list = []
    for data, label, bias_label in zip(data_list, batch_labels, bias_labels):
        ...
        nodes_score = node_masker[nodes_offset:nodes_offset + num_nodes]

        prune_num_nodes = int(num_nodes * args.pa)

        _, nodes_index = torch.sort(nodes_score.view(-1))

        prune_nodes_index = nodes_index[:prune_num_nodes]
        data.remove_nodes(prune_nodes_index)
        new_data_list.append((data, label, bias_label))
        offset += num_edges
        nodes_offset += num_nodes
        
    return new_data_list
def pruning_batch_data_from_mask_train(data_list, batch_labels, bias_labels, batch_index, data_mask, node_masker, args):

    offset = 0
    nodes_offset = 0
    new_data_list = []
    for data, label, bias_label, tmp_index in zip(data_list, batch_labels, bias_labels, batch_index):
        ...
        prune_nodes_index = nodes_index[:prune_num_nodes]
        data.remove_nodes(prune_nodes_index)
        new_data_list.append((data, label, bias_label, tmp_index))
        offset += num_edges
        nodes_offset += num_nodes
        
    return new_data_list
def masker_pruning_dataset_train(dataloader, masker, device, args):
    data_list = []
    with torch.no_grad():
        for iter, (batch_graphs, batch_labels, bias_label, batch_index) in enumerate(dataloader):
            ...
            batch_graphs_pruned = pruning_batch_data_from_mask_train(batch_graphs, batch_labels, bias_label, batch_index, data_mask, node_masker, args)
...
def masker_pruning_dataset(dataloader, masker, device, args):
    data_list = []
    with torch.no_grad():
        for iter, (batch_graphs, batch_labels, bias_label, ) in enumerate(dataloader):
            ...
            data_mask, node_masker = masker(batch_graphs, batch_x, batch_e)
            batch_graphs = dgl.unbatch(batch_graphs)
            batch_graphs_pruned = pruning_batch_data_from_mask(batch_graphs, batch_labels, bias_label, data_mask, node_masker, args)
            data_list += batch_graphs_pruned
